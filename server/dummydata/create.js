...
var promise = mongoose.connect(process.env.MONGODB_URI || process.env.MONGOLAB_URI, {
    useMongoClient: true,
});
...
function generateTours(N, categories,regions) {
    ...
    for (i = 0; i < N; i++) {
        ...
        for (j = 0; j < rundomNum; j++) {
            randomItems.push(categories[Math.floor(Math.random() * categories.length)]._id);
        }

        let randomRegion = regions[Math.floor(Math.random() * categories.length)]._id ;
        ...
        tours.push({
            ...
            content: {
                ...
                images:[faker.image.city(),faker.image.city(),faker.image.city(),faker.image.city(),faker.image.city()],
        ...
        if (regions.length > 0) {
            ...
            TourCategory.find({}).then((categories) => {
                if (categories.length > 0) {
                    ...
                    console.log(chalk.blue('categories == 0. We need to generate and create categories'));
