...
export const updatePages = (pagesIds, itemsPerPage) =>
  chunk(pagesIds, itemsPerPage).reduce((acc, cur, i) => {
    acc[i] = cur;
    return acc;
  }, {});
...
export function createBasicActions(
  ...
  const saveItem = (payload, isNew) => dispatch => {
    return dispatch({
      [middleware]: {
        types: [
          `${aliasPlural}_SAVE_REQUEST`,
          `${aliasPlural}_SAVE_SUCCESS`,
          `${aliasPlural}_SAVE_FAILURE`,
        ],
        method: isNew ? 'POST' : 'PUT',
        endpoint: isNew
          ? `/api/${endpoint}/`
          : `/api/${endpoint}/${payload._id}`,
        body: payload,
        toasterMsg: {
          success: 'Saved',
        },
  ...
  const deleteItems = ids => ({
    [middleware]: {
      types: [
        `${alias}_DELETE_REQUEST`,
        `${alias}_DELETE_SUCCESS`,
        `${alias}_DELETE_FAILURE`,
      ],
      method: 'DELETE',
      endpoint: `/api/${endpoint}`,
      toasterMsg: {
...
export const basicReducerEvents = {
  success: alias => (state, action) => {
    ...
    const allIdsResult =
      payload.result instanceof Array ? payload.result : payload.result.items;
    return {
      ...state,
      allIds: [...allIdsResult],
  ...
  itemSuccess: alias => (state, action) => {
    const payload = action.response;
    const allIds = [...state.allIds];
