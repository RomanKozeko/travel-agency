...
const fetchFilteredHotels = filtersQuery => ({
  [CALL_API]: {
    types: [
      'FILTERED_HOTELS_REQUEST',
      'FILTERED_HOTELS_SUCCESS',
      'FILTERED_HOTELS_FAILURE',
    ],
    method: 'GET',
...
const hotelsReducer = createReducer(defaultState, {
  ...
  [actions.HOTEL_SUCCESS]: state => ({
    ...state,
    isFetching: false,
    isSaving: false,
  }),
  [actions.HOTEL_SAVE_REQUEST]: state => ({ ...state, isSaving: true }),
  [actions.HOTEL_SAVE_SUCCESS]: state => ({
    ...state,
    isFetching: false,
    isSaving: false,
  }),
