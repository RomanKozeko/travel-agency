...
export const saveItem = ({ payload, fileType }) => dispatch => {
  ...
  return dispatch({
    [CALL_API]: {
      types: [
        MEDIAFILE_SAVE_REQUEST,
        MEDIAFILE_SAVE_SUCCESS,
        MEDIAFILE_SAVE_FAILURE,
      ],
      method: 'POST',
...
export const updateItemContent = makeActionCreator(
  'UPDATE_ITEM_CONTENT',
  'itemID',
  'langID',
  'val'
...
const mediaFilesReducer = createReducer(defaultState, {
  ...
  [actions.MEDIAFILES_DELETE_REQUEST]: state => ({
    ...state,
    isDeleting: true,
  }),
  [actions.MEDIAFILES_DELETE_SUCCESS]: reducerHelper.deleteSuccess,
  [actions.MEDIAFILES_DELETE_FAILURE]: state => ({
    ...state,
    isDeleting: false,
  }),
  [actions.MEDIAFILES_FAILURE]: state => ({ ...state, isFetching: false }),
  ...
  UPDATE_ITEM_CONTENT: (state, { itemID, langID, val }) => {
    ...
    newState.byIds[itemID].content = newState.byIds[itemID].content.map(
      contentItem => {
