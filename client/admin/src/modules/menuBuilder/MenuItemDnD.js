...
const MenuItemDnDContainer = ({
  ...
  return (
    <div>
      ...
      <li key={item._id}>
        <div className={hovered ? css(styles.item, styles.hoveredItem) : css(styles.item)}>
      ...
      {item.children.length > 0 &&
        <TreeItems tree={item.children} deleteEntity={deleteEntity} updateEntities={updateEntities} pages={pages}/>
      }
    </div>
  )
};
