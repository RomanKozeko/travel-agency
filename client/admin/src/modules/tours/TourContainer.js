...
class TourContainer extends React.Component {
    ...
    submit = (tour, isNew) => {
        isNew ? this.props.addTour(tour) : this.props.editTour(tour);
    };
                ...
                {!tour || isFetching ? (
                    ...
                    <div>
                        ...
                        <Portlet isBordered={true}>
                            <Tabs className={css(styles.tabs)} value={this.state.index} onChange={(event, index) => this.handleChange(event, index)}>
                                {this.props.languages.map(lang => (
                                    <Tab label={lang.title} key={lang._id} />
                                ))}
                            </Tabs>

                            <TourForm {...this.props} onSubmit={this.submit} selectedTabIndex={tabIndex} />
                        </Portlet>
                    </div>
                )}
            </div>
