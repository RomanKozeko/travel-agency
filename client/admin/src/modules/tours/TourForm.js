...
class TourForm extends Component {
    ...
    addTourProgram = langID => () => {
        const selectedPreview = [...this.props.selectedPreview];
        const contentByLang = { ...this.state.contentByLang };
        const programFile = contentByLang[langID].programFile ? [...contentByLang[langID].programFile] : [];
    ...
    deletePreviewItems = () => {
        ...
        const preview = tour.preview.filter(previewItem => !selectedPreviewItems.find(selectedItem => selectedItem._id === previewItem._id));
    ...
    saveProgram = (langID, programId) => program => {
        const contentByLang = { ...this.state.contentByLang };
        contentByLang[langID].programs[programId].program = program;
        this.setState({ contentByLang });
    };

    addNewProgram = langID => {
        const contentByLang = { ...this.state.contentByLang };
        const lastId = contentByLang[langID].programs.length;
        contentByLang[langID].programs.push({ id: lastId, program: [] });
        this.setState({ contentByLang });
    };

    deleteProgram = (langID, id) => {
        const contentByLang = { ...this.state.contentByLang };
        contentByLang[langID].programs.splice(id, 1);
        this.setState({ contentByLang });
    };
    ...
    toggleEnableForLanguage = id => (e, checked) => {
        ...
        tour.disabledForLanguages = checked ? [...tour.disabledForLanguages, id] : tour.disabledForLanguages.filter(item => item !== id);
            ...
            <form onSubmit={e => this.saveTour(e)}>
                <div className="row">
                    <div className="col-md-4">
                        ...
                        <Button onClick={this.deletePreviewItems} className={css(styles.button)} color="accent" variant="raised" disabled={!this.state.selectedPreviewItems.length}>
                            Удалить выбранные
                        </Button>
                        ...
                        <div className={css(styles.regionWrapper)}>
                            <TreeList selectedItems={tour.regions} items={this.props.regions} selectItems={this.selectRegions} />
                        </div>
                    </div>
                    <div className="col-md-8">
                        <FormControlLabel
                            control={<Switch checked={tour.enabled} onChange={(event, checked) => this.setState({ tour: { ...this.state.tour, enabled: checked } })} aria-label="checkedD" />}
                            label={tour.enabled ? 'Активный' : 'Неактивный'}
                        />

                        <TextField name="url" value={tour.url} onChange={this.handleInputChange(null, 'url')} fullWidth required className={css(styles.field)} label="url" />
                                ...
                                {selectedTabIndex === i && (
                                    <div>
                                        <FormControlLabel
                                            ...
                                            label={tour.disabledForLanguages && tour.disabledForLanguages.includes(lang._id) ? 'Скрыть для этого языка' : 'Скрыть для этого языка'}
                                        />
                                        <TextField
                                            name="title"
                                        ...
                                        <TextField
                                            ...
                                            fullWidth
                                        />
                                        <CollapseComponent title="Цены">
                                            <TextField
                                                name="price"
                                                type="number"
                                                value={contentByLang[lang._id].priceBYN || ''}
                                                onChange={this.handleInputChange(lang._id, 'priceBYN')}
                                                fullWidth
                                                className={css(styles.field)}
                                            ...
                                            <TextField
                                                name="price"
                                                type="number"
                                                value={contentByLang[lang._id].priceRUB || ''}
                                                onChange={this.handleInputChange(lang._id, 'priceRUB')}
                                                fullWidth
                                                className={css(styles.field)}
                                                label="Цена в руб для текущего языка"
                                            />
                                            <TextField
                                                name="price"
                                                type="number"
                                                value={contentByLang[lang._id].priceUSD || ''}
                                                onChange={this.handleInputChange(lang._id, 'priceUSD')}
                                                fullWidth
                                                className={css(styles.field)}
                                                label="Цена в долларах для текущего языка"
                                            />
                                            <TextField
                                                name="price"
                                                type="number"
                                                value={contentByLang[lang._id].priceEUR || ''}
                                                onChange={this.handleInputChange(lang._id, 'priceEUR')}
                                                fullWidth
                                                className={css(styles.field)}
                                                label="Цена в евро для текущего языка"
                                            />
                                            <TextField
                                                name="price"
                                                type="number"
                                                value={contentByLang[lang._id].pricePLN || ''}
                                                onChange={this.handleInputChange(lang._id, 'pricePLN')}
                                                fullWidth
                                                className={css(styles.field)}
                                                label="Цена в злотых для текущего языка"
                                            />
                                        </CollapseComponent>
                                        <div className={css(styles.selectWrapper)}>
                                            <FormControl>
                                                <InputLabel htmlFor="parentRegion" className={css(styles.select)}>
                                                    Тип питания
                                                </InputLabel>
                                                <Select
                                                    className={css(styles.select)}
                                                    value={typeof tour.food === 'object' ? tour.food._id : tour.food || '1'}
                                                    onChange={this.handleInputChange(null, 'food')}
                                                    input={<Input id="food" className={css(styles.select)} fullWidth />}
                                                    fullWidth
                                                >
                                                    <MenuItem className={css(styles.select)} key="1" value={'noParent'}>
                                                        ...
                                                    </MenuItem>
                                                    {this.props.food.map(foodItem => {
                                                        return (
                                                            <MenuItem key={foodItem._id} value={foodItem._id}>
                                                                {foodItem.content[0].title}
                                                            </MenuItem>
                                                        );
                                                    })}
                                                </Select>
                                        ...
                                        <Button onClick={() => this.addNewProgram(lang._id)} className={css(styles.button)}>
                                            {'Add program'}
                                        </Button>
                                        {contentByLang[lang._id].programs.map(item => (
                                            <CollapseComponent title="Программа тура">
                                                {item.id !== 0 && (
                                                    <Button onClick={() => this.deleteProgram(lang._id, item.id)} className={css(styles.button)}>
                                                        {'Delete program'}
                                                    </Button>
                                                )}
                                                <TourProgram days={item.program} save={this.saveProgram(lang._id, item.id)} />
                                            </CollapseComponent>
                                        ))}
                                        <AddTourPreviewPopup filesType={'@docs'} label={'Загрузить программу тура'} addPreview={this.addTourProgram(lang._id)} />
                                        {contentByLang[lang._id].programFile
                                            ? contentByLang[lang._id].programFile.map(item => (
                                                  <div key={item._id} className={css(styles.fileItem)}>
                        ...
                        <div>
                            <CollapseComponent title="Отели">
                                <HotelsFilterContainer selectedItems={tour.hotels} toggleItem={this.toggleItem('hotels')} />
                            </CollapseComponent>
                            <CollapseComponent title="Достопримечательности">
                                <ShowplacesFilterContainer selectedItems={tour.showplaces} toggleItem={this.toggleItem('showplaces')} />
                            </CollapseComponent>
                        </div>
                    </div>
                </div>
                {!this.state.isValidForm && <NotificationPanel>Пожалуйста, заполните все обязательные поля для всех языков</NotificationPanel>}
                <br />
                <Button variant="raised" type="submit" color="primary" className={css(styles.submitButton)} disabled={isSaving}>
