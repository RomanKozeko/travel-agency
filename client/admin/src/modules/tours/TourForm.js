...
class TourForm extends Component {
  ...
  handleInputChange = (langID, name) => event => {
    if (langID) {
      ...
      this.setState({
        tour: { ...this.state.tour, [name]: event.target.value },
      });
    }
  };
  ...
  addPreview = () => {
    ...
    selectedPreview.forEach(selectedItem => {
      const isNotExist = preview.every(
        previewItem => previewItem._id !== selectedItem
      );
      if (isNotExist) {
        preview.push({
          ...this.props.mediaFiles.byIds[selectedItem],
          active: false,
        });
      }
    });
  ...
  addTourProgram = langID => () => {
    ...
    selectedPreview.forEach(selectedItem => {
      const isNotExist = programFile.every(
        previewItem => previewItem._id !== selectedItem
      );
      if (isNotExist) {
        programFile.push({
  ...
  deleteProgramFile = (langID, id) => () => {
    const contentByLang = { ...this.state.contentByLang };
    const programFile = contentByLang[langID].programFile.filter(
      item => item._id !== id
    );
      ...
      <form onSubmit={e => this.saveTour(e)}>
        <div className="row">
          <div className="col-md-4">
            ...
            <ImageGridList
              imgs={tour.preview}
              clickHandler={this.togglePreviewItem}
            />
              ...
              control={
                <Switch
                  ...
                  onChange={(event, checked) =>
                    this.setState({
                      tour: { ...this.state.tour, enabled: checked },
                    })
                  }
                  aria-label="checkedD"
                />
              ...
              <div key={lang._id}>
                {selectedTabIndex === i && (
                  <div>
                    <FormControlLabel
                      control={
                        <Switch
                          checked={
                            tour.disabledForLanguages &&
                            tour.disabledForLanguages.includes(lang._id)
                          }
                    ...
                    <Button
                      onClick={() => this.addNewProgram(lang._id)}
                      className={css(styles.button)}
                    >
                      {'Add program'}
                    </Button>
                    {contentByLang[lang._id].programs.map(item => (
                      <CollapseComponent title="Программа тура">
                        {item.id !== 0 && (
                          <Button
                            onClick={() =>
                              this.deleteProgram(lang._id, item.id)
                            }
                            className={css(styles.button)}
                          >
                            {'Delete program'}
                        ...
                        <TourProgram
                          days={item.program}
                          save={this.saveProgram(lang._id, item.id)}
                        />
                      </CollapseComponent>
                    ))}
                    <AddTourPreviewPopup
                    ...
                    {contentByLang[lang._id].programFile
                      ? contentByLang[lang._id].programFile.map(item => (
                          <div key={item._id} className={css(styles.fileItem)}>
                            <span>{item.filename.slice(13)}</span>
                            <Icon
                              onClick={this.deleteProgramFile(
                                lang._id,
                                item._id
                              )}
                            >
        ...
        {!this.state.isValidForm && (
          <NotificationPanel>
            Пожалуйста, заполните все обязательные поля для всех языков
          </NotificationPanel>
        )}
        <br />
        <Button
