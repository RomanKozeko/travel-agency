...
class FilterByRegions extends Component {
  ...
  selectRegions = e => {
    const updatedItems = this.toggleItem(
            ...
            {filteredItems.map(
              (filteredItem, i) =>
                !this.props.selectedItems.find(
                  selectedItemId => filteredItem._id === selectedItemId
                ) && (
                  <div
                    key={filteredItem._id}
                    className={css(styles.filteredItem)}
                  >
                    <div className={css(styles.itemLink)}>
                      <Link
                        to={`/admin/${itemsNameToFilter}/${filteredItem._id}`}
                      >
                        {filteredItem.content[0].title}
                      </Link>
                    </div>
