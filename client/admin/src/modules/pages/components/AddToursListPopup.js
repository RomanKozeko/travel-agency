...
class AddToursListPopup extends Component {
  constructor(props) {
    ...
    if (
      filtersObj.categories &&
      filtersObj.categories[0] &&
      !filtersObj.categories[0]._id
    ) {
      filtersObj.categories = this.props.categories.filter(category =>
        filtersObj.categories.includes(category._id)
      );
    }

    if (
      filtersObj.regions &&
      filtersObj.regions[0] &&
      !filtersObj.regions[0]._id
    ) {
      filtersObj.regions = this.props.regions.filter(category =>
        filtersObj.regions.includes(category._id)
      );
    }
  ...
  onFilterSelect = (filterType, filters) => {
    ...
    this.setState(prevState => ({
      [prevState.filtersObj[filterType]]: filters,
    }));
  };
    ...
    return (
      <Dialog open={isOpen} onRequestClose={handleRequestClose}>
        ...
        <DialogActions>
          ...
          <Button
            onClick={() => saveRow('', 'tours', this.state)}
            color="primary"
          >
            Сохранить
          </Button>
