...
const getPrice = ({ price, priceBYN, priceRUB, priceEUR, pricePLN, priceUSD, currency, currencies }) => {
    ...
    if (!priceBYN && !priceRUB && !priceEUR && !pricePLN && !priceUSD) {
        ...
        return `${(((price || 0) / currencyWithRate.Cur_OfficialRate) * currencyWithRate.Cur_Scale).toFixed(0) || '0'} ${currencyWithRate.Cur_Abbreviation}`;
    }
...
class Tour extends Component {
    ...
    render() {
        ...
        const { price, priceBYN, priceRUB, priceEUR, pricePLN, priceUSD } = tour ? tour.content : {};
        return (
            <div>
                                ...
                                <div className="col-md-9">
                                    <div className={css(styles.content)}>
                                        ...
                                        <table>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <b>{tour.food && getContentByLanguage(tour.food.content, languageID).title}</b>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td className={css(styles.cell)}>{window.TA.content.price}:</td>
                                                <td>
                                                    <div className={css(styles.price)}>{getPrice({ price, priceBYN, priceRUB, priceEUR, pricePLN, priceUSD, currency, currencies })}</div>
                                                </td>
                                            </tr>
                                        </table>
                                    </div>
                                    ...
                                    {tour.content.programs &&
                                        tour.content.programs.map((item, index) => (
                                            <div className={css(styles.content)}>
                                                <FancyHeader title={`${window.TA.content.tourProgram} ${index+1}`} />
                                    ...
                                    <div className={css(styles.content)}>
                                        ...
                                        <ItemsGallery>
                                            {tour.showplaces.map(item => (
                                                <HotelPreview key={item._id} item={item} url="showplaces" content={getContentByLanguage(item.content, languageID)} />
                                            ))}
                                        </ItemsGallery>
                                    </div>
                                    ...
                                    <div className={css(styles.content)}>
                                        ...
                                        <ItemsGallery>
                                            {tour.hotels.map(hotel => (
                                                <HotelPreview key={hotel._id} item={hotel} url="hotels" content={getContentByLanguage(hotel.content, languageID)} />
                                            ))}
                                        </ItemsGallery>
                                    </div>
