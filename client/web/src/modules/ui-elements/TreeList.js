...
const TreeList = ({items, onChange}) => {
  ...
  return (
    <ul className={css(styles.ul)}>
      {populatedItems.map(item => <TreeItem item={item} onChange={onChange} /> )}
    </ul>
  );
};
...
let TreeItem = ({item, onChange, togglePanel, isOpen, inner}) => (
	<div>
		<div className={css(styles.item)}>
			...
			{
				item.children &&
				<div onClick={togglePanel} className={isOpen ? css(styles.rotate, styles.pointer) : css(styles.pointer)}>
		...
		</div>
		{
			item.children && isOpen &&
				item.children.map(child => <TreeItem item={child} onChange={onChange} inner={true}/>)
		}
	</div>
);
